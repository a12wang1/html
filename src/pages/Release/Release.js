...
const { getLevel,checkPerson, publishtask} = options;
    ...
    submit = () => {
        ...
        this.formRef.validateAll(async (error, value) => {
            if (error) {
                ...
                let data = {};
                 let images = JSON.stringify(_this.Img);
                ...
                let result  = await publishtask(data);
                if (result.code == 1){
       ...
       return (
            <select ref={e => _this.leve = e} onChange={_this.selected}>
                {_this.state.result.map(item => (
                    <option value={item.integral} key={item.id}>{item.name}</option>
                ))}
